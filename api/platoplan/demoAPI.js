	...
	case 'videoByScript':
		...
		_f['oraginB'] = function(cbk) {
			pkg.fs.stat(tmp_plugOrg2, function(err, stat) {
				if(!err) { cbk(tmp_plugOrg2);
					...
					var childProcess = require('child_process');
					var ls = childProcess.exec('ffmpeg  -i ' + file_video + ' -ss '+ 11 + ' -t ' + CP.data.videoLength + ' -c copy ' + tmp_plugOrg2 +' -y ', 		   
						function (error, stdout, stderr) {
							cbk(true);
						});
				}
		...
		_f['plugIn'] = function(cbk) {
			pkg.fs.stat(tmp_plugin, function(err, stat) {
				if(!err) { cbk(tmp_plugin);
					...
					var childProcess = require('child_process');
					var ls = childProcess.exec('ffmpeg  -i ' + plugin_video + ' -ss '+ 0 + ' -t ' + 5 + ' -c copy ' + tmp_plugin +' -y ', 		   
						function (error, stdout, stderr) {
							cbk(true);
						});
				}
		...
		*/
		...
		_f['output'] = function(cbk) {
			...
			var ls = childProcess.exec('ffmpeg -f concat  -i ' + tmp_combine + ' -vcodec copy -acodec copy ' + tmp_output + ' -y',
				function (error, stdout, stderr) {
					cbk(true);
				});
		};		
